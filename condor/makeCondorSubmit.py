    ...
    #for year in ["2016"]:
        for tag in Tags:
            if "vjets" in data or "vv" in data:
                condorout = "mkdir /storage/af/user/idutta/HH/CMSSW_9_4_2/src/V3/HHLooper/condor/condor_output/"+tag[0]+"_others"
                os.system(condorout)
                condorout = "mkdir /storage/af/user/idutta/HH/CMSSW_9_4_2/src/V3/HHLooper/condor/condor_output/"+tag[0]+"_others/"+year
                os.system(condorout)
            else:
                condorout = "mkdir /storage/af/user/idutta/HH/CMSSW_9_4_2/src/V3/HHLooper/condor/condor_output/"+tag[0]+"_"+data
                os.system(condorout)
                condorout = "mkdir /storage/af/user/idutta/HH/CMSSW_9_4_2/src/V3/HHLooper/condor/condor_output/"+tag[0]+"_"+data+"/"+year
                os.system(condorout)
...
for year in ["2016","2017","2018"]:
    condorout = "mkdir /storage/af/user/idutta/HH/CMSSW_9_4_2/src/V3/HHLooper/condor/condor_output/yield_AN_sr_sys_180122_300_nominal_nosys_data"
    os.system(condorout)
    condorout = "mkdir /storage/af/user/idutta/HH/CMSSW_9_4_2/src/V3/HHLooper/condor/condor_output/yield_AN_sr_sys_180122_300_nominal_nosys_data/"+year
    os.system(condorout)
    condorSubmit = "condor_submit/submitCondor_data_yield_AN_sr_sys_180122_300_nominal_nosys_"+year
    jobName      = str(date.today())+"_data_yield_AN_sr_sys_180122_300_nominal_nosys_"+year
    outHistFile = "data.root"
    isData       =  "1"
    ...
    for line in fileinput.FileInput(condorSubmit, inplace=1):
        ...
        line=line.replace("INPUTDIR",inputDir)
        line=line.replace("OUTFILENAME",outHistFile)
        line=line.replace("TAG","yield_AN_sr_sys_180122_300_nominal_nosys")
        line=line.replace("ISDATA",isData)
