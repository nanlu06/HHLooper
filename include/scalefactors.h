	...
	float getScaleFactors_VBF(string year, float pt)
	  ...
	  return result;
	    ...
	    if(xbb < 0.9) idx_xbb  = 0;
            ...
            if (year == "2016")
            {
	      float sf[6] = {0.993, 0.982, 1.011, 0.964, 0.846, 0.916};
	      float esf[6] = {0.015, 0.014, 0.083, 0.054, 0.074,0.089};
               return sf[idx_xbb] + type*esf[idx_xbb];
            }
            if (year == "2017")
            {
	      float sf[6] = {1.006, 1.012, 1.034, 1.022, 0.976, 1.047};
	      float esf[6] = {0.015,0.014, 0.079, 0.054, 0.076, 0.084};
               return sf[idx_xbb] + type*esf[idx_xbb];
            }
            if (year == "2018")
            {
	      float sf[6] = {0.982, 0.986, 1.066, 0.953, 0.930, 0.825};
	      float esf[6] = {0.016, 0.015, 0.063, 0.041, 0.058, 0.058};
               return sf[idx_xbb] + type*esf[idx_xbb];
            }
            return 1.0;
        }
