...
Double_t fitfrf_up(Double_t *x,Double_t *par) {
    ...
    TMatrixD tmp1(1,npars),err1(1,1);
    tmp1.Mult(c,cov_data_mbkg_mc);
    err1.Mult(tmp1,cT);
    //err1.Print();
    //cout << err1.Determinant() <<endl;
    
    TMatrixD tmp2(1,npars),err2(1,1);
    ...
    Double_t fitval = par[0];
    if(x[0]<300){
        fitval = par[0] + par[1]*x[0] + sqrt(err1.Determinant()) ;
    }
    else{
        fitval = par[0] + 300.*par[1] - 300.*par[2] + par[2]*x[0] + sqrt(err2.Determinant()) ;
    }
    return fitval;
}

Double_t fitfrf_dn(Double_t *x,Double_t *par) {
    ...
    Double_t fitval = par[0];
    if(x[0]<300){
        fitval = par[0] + par[1]*x[0] - sqrt(err2.Determinant()) ;
    }
    else{
        fitval = par[0] + 300.*par[1] - 300.*par[2] + par[2]*x[0] - sqrt(err1.Determinant()) ;
    }
    return fitval;
}
...
void RampModel_alt() {
    ...
    float xmin = 0;
    float xmax = 1000.;
    TF1 *func = new TF1("fit",fitfrf,xmin,xmax,npars);
    ...
    cov_data_mc.Print();
    //double *cov_data_mc_ele = cov_data_mc.GetElements();
    //cout <<"cov_data_mc_ele "<<cov_data_mc_ele[0]<< " "<< cov_data_mc_ele[1]<<endl;
    ...
    cv->SaveAs(year+"_fit_piecewise_data_MC_altmodel.png");
    ...
    TF1 *fit_2_up = new TF1("fit_2_up",fitfrf_up,xmin,xmax,npars);
    fit_2_up->SetLineColor(kBlue); 
    fit_2_up->SetLineStyle(2); 
    fit_2_up->SetParameters(fit_2->GetParameter(0), fit_2->GetParameter(1), fit_2->GetParameter(2));
    for(int k=0; k<npars; k++){
        cout <<"par for fit_2_up "<<k<<" : "<<fit_2_up->GetParameter(k)<<"+/-"<<fit_2_up->GetParError(k)<<endl;
    }

    TF1 *fit_2_dn = new TF1("fit_2_down",fitfrf_dn,xmin,xmax,npars);
    fit_2_dn->SetLineColor(kBlue);
    fit_2_dn->SetLineStyle(2); 
    fit_2_dn->SetParameters(fit_2->GetParameter(0), fit_2->GetParameter(1), fit_2->GetParameter(2));
    for(int k=0; k<npars; k++){
    ...
    fit_2_up->Draw("same");
    fit_2_dn->Draw("same");
    cv2->SaveAs(year+"_fit_piecewise_data_mMC_altmodel.png");
